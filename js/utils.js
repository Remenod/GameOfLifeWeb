...
export async function updateUrlParams() {
    ...
    if (height)
        params.set("h", height);
    if (rule)
        params.set("r", rule.replace("/", "."));
    if (mask)
        params.set("m", mask == defMask ? "d" : encode_field(mask, true));
    if (field)
        params.set("f", field.replace(/\[/g, '-').replace(/\]/g, '_').replace(";", '.'));
...
export function loadFromUrlParams() {
    ...
    if (rule)
        ruleInputEl.value = rule.replace(".", "/");
    if (field)
        fieldInputEl.value = field.replace(/-/g, '[').replace(/_/g, "]").replace(".", ";");
    if (mask)
        setNeighborMask(mask == "d" ? defMask : decode_field(mask, true));
}
